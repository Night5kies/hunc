import { getPayloadClient } from "@/lib/payload";
import PastEventsClient, { type PastEvent } from "./PastEventsClient";

function WorkInProgress() {
  return (
    <section className="min-h-[60vh] px-6 py-20 bg-white">
      <div className="max-w-3xl mx-auto text-center">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 border-b-4 border-green-600 inline-block text-black">
          Past Events
        </h1>
        <p className="text-lg text-gray-700 mt-6">Past events are a work in progress. Check back soon.</p>
      </div>
    </section>
  );
}

export default async function PastEventsPage() {
  try {
    const payload = await getPayloadClient();
    const result = await payload.find({
      collection: "past-events",
      where: {
        _status: {
          equals: "published",
        },
      },
      sort: "sortOrder",
      limit: 100,
      depth: 1,
    });

    const events: PastEvent[] = result.docs
      .map((doc) => {
        const mappedImages = (doc.images || [])
          .map((image: unknown) => {
            if (image && typeof image === "object" && "url" in image && image.url) {
              return image.url;
            }
            return null;
          })
          .filter((image: string | null): image is string => Boolean(image));

        return {
          title: doc.title,
          description: doc.description,
          images: mappedImages,
        };
      })
      .filter((event) => event.images.length > 0);

    if (events.length === 0) {
      return <WorkInProgress />;
    }

    return <PastEventsClient events={events} />;
  } catch {
    return <WorkInProgress />;
  }
}
