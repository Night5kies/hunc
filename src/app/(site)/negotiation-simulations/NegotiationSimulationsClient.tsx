...
export default function NegotiationSimulationsClient({ events }: { events: NegotiationSimulation[] }) {
  const [selectedEvent, setSelectedEvent] = useState<NegotiationSimulation | null>(null);
      ...
      <ResponsiveSection background="white" padding="lg" className="min-h-screen bg-cover bg-center">
        <ResponsiveContainer maxWidth="6xl">
            ...
            {[...events].reverse().map((event) => (
              ...
              >
                <div className="relative h-64 w-full">
                  <Image src={event.image.url} alt={event.image.alt} fill className="object-cover" />
                  ...
                  <h2 className="absolute bottom-4 left-4 text-2xl font-semibold text-white drop-shadow-lg">{event.title}</h2>
                </div>
                <div className="p-6 border-t border-gray-100">
                  <p className="text-lg-paragraph line-clamp-7">{event.description}</p>
                </div>
              </m.article>
            ))}
          </m.div>
          ...
          {selectedEvent && (
            ...
            >
              <div
                className="bg-white ring-2 ring-gray-500 rounded-2xl overflow-hidden max-w-4xl w-full relative max-h-[90vh] overflow-y-auto"
              ...
              >
                ...
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="w-full md:w-1/2 p-4 md:p-6 flex flex-col justify-center">
                    <h2 className="text-xl sm:text-2xl font-semibold mb-4">{selectedEvent.title}</h2>
                    <p className="text-base sm:text-lg text-gray-600 leading-relaxed">{selectedEvent.description}</p>
                  </div>
                </div>
              </div>
            </div>
