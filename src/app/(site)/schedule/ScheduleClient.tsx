...
export type ScheduleEventsByMonth = Record<string, Omit<ScheduleEvent, "month">[]>;
...
export default function ScheduleClient({ eventsByMonth }: { eventsByMonth: ScheduleEventsByMonth }) {
  return (
      ...
      <ResponsiveSection background="white" padding="lg" className="min-h-screen relative">
        ...
        <ResponsiveContainer maxWidth="4xl" className="relative">
          ...
          <div className="space-y-12">
            {Object.entries(eventsByMonth).map(([month, monthEvents], monthIndex) => (
              <m.section
                ...
                className="space-y-6"
              >
                <m.h2 variants={cardVariants} className="text-2xl md:text-3xl font-semibold text-gray-800 border-l-4 border-green-600 pl-4">
                  {month}
                </m.h2>
                ...
                <div className="grid gap-4 md:gap-6">
                  {monthEvents.map((event, index) => (
                    <m.div key={`${event.date}-${index}`} variants={cardVariants}>
                      <ResponsiveCard variant="default" className="transform transition hover:scale-[1.02] hover:shadow-lg">
                        <div className="flex flex-col md:flex-row md:items-center gap-4">
                          <div className="flex-shrink-0 md:w-48">
                            <div className="text-lg font-semibold text-gray-900">
                              {event.date}
                              {event.time && <div className="text-sm font-normal text-gray-600">{event.time}</div>}
                            </div>
                          </div>
                          ...
                          <div className="flex-grow">
                            ...
                            {(event.location || event.description) && (
                              <div className="mt-2 text-gray-600">
                                {event.location && <p className="text-sm font-medium">Location: {event.location}</p>}
          ...
          >
            ...
            <div className="grid gap-4 md:gap-6">
              <ResponsiveCard variant="default" className="border-2 border-green-100 bg-gradient-to-r from-green-50/50 to-white">
